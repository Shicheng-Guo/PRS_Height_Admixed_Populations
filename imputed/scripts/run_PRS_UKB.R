...
PolScore2<- function(data=vcf2){
        data-> hei2
        samps<-colnames(hei2)[10:ncol(hei2)]
...
W<-as.numeric(args[1])
P<-as.numeric(args[2])
vec_all<-readRDS(paste0('~/height_prediction/imputed/output/UKB_vec_all_',W, '_', P,'.Rds'))

cat('LOADED\n')
all_prs<-vector('list', 2)
names(all_prs)<-c('UKB_afr', 'UKB_eur')
all_prs[['UKB_afr']]<-vector('list',22)
all_prs[['UKB_eur']]<-vector('list',22)

for(chr in 22:1){
	betas<-vec_all[[chr]]
	vcf<-fread(paste0('zcat ~/height_prediction/imputed/output/UKB_afr_', W, '_',P, '_chr_', chr, '.vcf.gz'), fill=T)
	colnames(vcf)[1]<-"CHR"
	setkey(betas, CHR,POS)
	setkey(vcf, CHR, POS)
	vcf2<-betas[vcf,nomatch=0]
	all_prs[['UKB_afr']][[chr]]<-PolScore2(data=vcf2)
	remove(vcf,vcf2)
	vcf<-fread(paste0('zcat ~/height_prediction/imputed/output/UKB_eur_', W, '_', P, '_chr_', chr, '.vcf.gz'), fill=T)
        colnames(vcf)[1]<-"CHR"
        setkey(betas, CHR,POS)
        setkey(vcf, CHR, POS)
        vcf2<-betas[vcf,nomatch=0]
        all_prs[['UKB_eur']][[chr]]<-PolScore2(data=vcf2)
	cat(chr, ' done\n')
	}

#combine

samps<-names(all_prs[['UKB_afr']][[1]]) #sum PGS across chromosomes.
samps2<-names(all_prs[['UKB_eur']][[1]])
PGS2<-vector('list',2)
...
saveRDS(all_prs, file=paste0('~/height_prediction/imputed/output/PGS1', W, '_', P, '_UKB.Rds'))
saveRDS(PGS2, file=paste0('~/height_prediction/imputed/output/PGS2', W, '_', P, '_UKB.Rds'))
