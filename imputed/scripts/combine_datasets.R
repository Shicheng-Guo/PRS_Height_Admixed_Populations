...
for(I in names(B_HRS_afr)){ 
	...
	ALL$Dataset<-factor(ALL$Dataset, levels=c("UKB_afr", "HRS_afr",  "HRS_eur"))
	...
	geom_errorbarh(aes(x=Med_Eur_Anc, group=Dataset, colour=Dataset, xmin=HVB_L, xmax=HVB_U), width=0.05, size=0.5) +  scale_color_manual(values=c(brewer.pal(4, 'Set1')[c(1,4)],"#101010")) + 
...
for(I in names(B_HRS_afr)){
	...
	ALL2[[I]]$Dataset<-factor(ALL2[[I]]$Dataset, levels=c("UKB_afr", "HRS_afr",  "HRS_eur"))
...
for(I in names(B_HRS_afr)){
	...
	ALL2b[[I]]$Dataset<-factor(ALL2b[[I]]$Dataset, levels=c("UKB_afr", "HRS_afr",  "HRS_eur"))
