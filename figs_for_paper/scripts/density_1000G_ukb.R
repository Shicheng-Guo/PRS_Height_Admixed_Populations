...
#********************************
samples<-fread('/project/mathilab/data/1kg/20130502_phase3_final/integrated_call_samples_v3.20130502.ALL.panel', fill=T, header=T)[,.(sample, pop, super_pop)]
...
hei<-vector('list', 22)
for(Z in 1:22){
	fread(paste0('zcat /project/mathilab/bbita/gwas_admix/height/LD_prun/hei_SNPs_', args[1], '_chr', Z, '.vcf.gz'))-> hei[[Z]]
...
names(hei)<-seq(1:22)
saveRDS(hei, file=paste0('~/height_prediction/gwas/ukb_eur/output/hei_', args[1], '_1000g.Rds'))
...
for (P in pops){
	...
	PGS[[P]]<-vector('list', 22)
		...
		print(paste0("Chromosome is ", CR))
                PolScore_1000G(POP=P, CHR=CR, superpop=F, my_dt=args[1])-> PGS[[P]][[CR]]
                cat(paste0(CR, '  done\n'))
        }
        cat (paste0(P, ' done\n'))
        gc()
...
for (P in superpops){
	...
	cat(paste0("Super-population is ", P, '\n'))
        for(CR in 1:22){
		print(paste0("Chromosome is ", CR))
                PolScore_1000G(CHR=CR, POP=P, superpop=T, my_dt=args[1])-> PGS[[P]][[CR]]
                cat(paste0(CR, '  done\n'))
	}
        cat(paste0(P, ' done\n'))
        gc()
...
saveRDS(PGS2, file=paste0('~/height_prediction/gwas/ukb_eur/output/PGS_1000G_', args[1], '.Rds'))
...
PGS<-readRDS(paste0('~/height_prediction/gwas/ukb_eur/output/PGS_1000G_', args[1], '.Rds'))
...
ggsave(paste0(path,'PGS_1000G_', args[1], '_pops_UKB.png'))
...
ggsave(paste0(path, 'PGS_1000G_', args[1], '_superpops_UKB.png'))
...
ggsave(paste0(path,'PGS_1000G_', args[1], '_EUR_AFR_UKB.png'))
...
ggsave(paste0(path,'PGS_1000G_', args[1], '_EUR_UKB.png'))
...
ggsave(paste0(path, 'PGS_1000G_', args[1], '_AFR_UKB.png'))
...
ggsave(paste0(path, 'PGS_1000G_', args[1], '_AMR_UKB.png'))
...
ggsave(paste0(path, 'PGS_1000G_', args[1], '_SAS_UKB.png'))
...
ggsave(paste0(path,'PGS_1000G_', args[1], '_EAS_UKB.png'))
...
ggsave(paste0(path,'PGS_1000G_', args[1], '_pops_GIANT.png'))
...
ggsave(paste0(path, 'PGS_1000G_', args[1], '_superpops_GIANT.png'))
...
ggsave(paste0(path,'PGS_1000G_', args[1], '_EUR_AFR_GIANT.png'))
...
ggsave(paste0(path,'PGS_1000G_', args[1], '_EUR_GIANT.png'))
...
ggsave(paste0(path, 'PGS_1000G_', args[1], '_AFR_GIANT.png'))
...
ggsave(paste0(path, 'PGS_1000G_', args[1], '_AMR_GIANT.png'))
...
ggsave(paste0(path, 'PGS_1000G_', args[1], '_SAS_GIANT.png'))
...
ggsave(paste0(path,'PGS_1000G_', args[1], '_EAS_GIANT.png'))
print('Done making figures')


##
my_plotA<-ggplot(dat[group %in% superpops],aes(x=y1, fill=group)) + geom_density(alpha=0.30) + labs(x = "Polygenic Score") + labs(title = "UK Biobank") + scale_color_brewer(palette="PRGn") + coord_cartesian(xlim=c(-12,3)) + theme(legend.position="none")
my_plotB<-ggplot(dat_giant[group %in% superpops],aes(x=y1, fill=group)) + geom_density(alpha=0.30) + labs(x = "Polygenic Score") + labs(title ="GIANT") + scale_color_brewer(palette="PRGn") + coord_cartesian(xlim=c(-12,3)) + theme(legend.position="bottom")
pdf(paste0('~/height_prediction/figs_for_paper/figs/SM_S1_', args[1], '.pdf'), width=9, height=7)
