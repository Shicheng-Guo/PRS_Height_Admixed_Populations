...
for(I in names(Nr_SNPs)){
	readRDS(paste0('~/height_prediction/sib_betas/WHI/output/hei_', I, '.Rds'))-> smtg
...
for(I in 1:length(PGS2_WHI)){
        ...
        png(filename=paste0('~/height_prediction/sib_betas/WHI/figs/WHI_ggpairs_', names(PGS2_WHI)[I], '.png'))
        print(A)
...
for (I in names(PGS3_WHI)){
        ...
        print(myp)
        ggsave(paste0('~/height_prediction/sib_betas/WHI/figs/error_bars_', I, '.png'))
}
