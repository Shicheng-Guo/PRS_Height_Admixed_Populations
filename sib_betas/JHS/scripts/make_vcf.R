...
system(paste0('bash ', dir, 'scripts/master.sh ', dir, args[1]))
print('so far so good')
Sys.sleep(10)
system(paste0('bash ', dir, 'scripts/combine_temp.bash ', dir))
Sys.sleep(60)
this<-paste0('for K in {12..22}; do bsub -o ', dir, 'logs/logthis -e' , dir, 'logs/logthis -M 12000 Rscript --vanilla ', dir, 'scripts/giant_vcf_v2.R $K; done')
system(this)
andthis<-paste0('for K in {3..11}; do bsub -M 10240 -o ', dir, 'logs/logandthis -e ', dir, 'logs/logandthis Rscript --vanilla ', dir, 'scripts/giant_vcf_v2.R $K; done')
system(andthis)
andthis2<-paste0('for K in {1..2}; do bsub -M 15240 -o ', dir, 'logs/logandthis2 -e ', dir, 'logs/logandthis2 Rscript --vanilla ',dir,'scripts/giant_vcf_v2.R $K; done')
system(andthis2)
#******
#END **
#******
