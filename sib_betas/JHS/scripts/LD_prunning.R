...
if(opt$method == "LD"){
	try(LD_prun(dis=opt$method, W=opt$windowsize, p_thresh=opt$pvalue, CHR=opt$chromosome, r2=opt$r2))->res
	
} else {
	try(LD_prun(dis=opt$method, W=opt$windowsize, p_thresh=opt$pvalue, CHR=opt$chromosome))->res
}
...
#	setwd('/project/mathilab/bbita/gwas_admix/sib_gwas/JHS/')
	...
	saveRDS(res[['AFR']], file=paste0(parentdir, "prunned_1kg/LD_prunned_hei_chr", opt$chromosome, '_', opt$method, '_', opt$windowsize, '_', opt$pvalue, '_AFR.Rds'))
	saveRDS(res[['EUR']], file=paste0(parentdir, "prunned_1kg/LD_prunned_hei_chr", opt$chromosome, '_', opt$method, '_', opt$windowsize, '_', opt$pvalue, '_EUR.Rds'))
} else {
	saveRDS(res,file=paste0(parentdir, "prunned_1kg/LD_prunned_hei_chr", opt$chromosome, '_', opt$method, '_', opt$windowsize, '_', opt$pvalue, '.Rds'))
}
