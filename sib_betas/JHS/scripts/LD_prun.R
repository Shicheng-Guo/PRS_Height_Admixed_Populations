...
LD_prun<-function(W=NULL,p_thresh=NULL, dis=opt$method, CHR=opt$chromosome, r2=NULL){
	as.numeric(W)-> W;as.numeric(p_thresh)-> p_thresh
	readRDS(paste0(parentdir, "output/hei_ALL_chr", CHR, '.Rds'))-> hei
	cat('checkpoint 1\n')
	hei[order(p)]-> tp
	cat('checkpoint 2\n')
	remove(hei)
	...
	if (dis =='phys'){   
		...
		system('cd plink2')
		setwd(paste0(parentdir, "plink2/output/"))
