...
for (A in names(prs)){
	prs[[A]]<-vector('list',22)
		for(chr in 1:22){
		prs[[A]][[chr]]<-readRDS(paste0('~/height_prediction/loc_anc_analysis/output/chr', chr,'_',A, '_prs_WHI_', args[1],'.Rds'))
		cat(chr, '\r')
	}
cat(A, '\n')
}
...
for(I in my_cols){
	part_r2[[I]]<-partial.R2(lm(HEIGHTX~AGE+AGE2+EUR_ANC, data=final2), lm(HEIGHTX~AGE+AGE2+EUR_ANC+get(I), data=final2))
}
	
saveRDS(final2, paste0('~/height_prediction/loc_anc_analysis/output/all_PRS_WHI_', args[1], '.Rds'))

saveRDS(part_r2, paste0('~/height_prediction/loc_anc_analysis/output/part_R2_WHI_', args[1], '.Rds'))
txtStop()
###################
####
