...
prs<-vector('list', 22)
for(chr in 1:22){
	prs[[chr]]<-readRDS(paste0('~/height_prediction/loc_anc_analysis/output/chr', chr,'_0_prs_WHI_EuR_', args[1],'.Rds'))
}
...
prs<-vector('list', 22)
for(chr in 1:22){
        prs[[chr]]<-readRDS(paste0('~/height_prediction/loc_anc_analysis/output/chr', chr,'_0_prs_JHS_EuR_', args[1], '.Rds'))
}
samp_names<-names(prs[[1]])
dt<-data.table(SUBJID=samp_names, PRS_0=NA)
...
r2_jhs_afr<-partial.R2(lm(HEIGHTX~SEX+AGE+AGE2+EUR_ANC, data=final2), lm(HEIGHTX~SEX+AGE+AGE2+EUR_ANC+PRS_0, data=final2)) # 0.86%
#
saveRDS(final2, paste0('~/height_prediction/loc_anc_analysis/output/all_PRS_JHS_Eur_', args[1], '.Rds'))
...
prs<-vector('list', 22)
for(chr in 1:22){
        prs[[chr]]<-readRDS(paste0('~/height_prediction/loc_anc_analysis/output/chr', chr,'_0_prs_UKB_afr_EuR_', args[1], '.Rds'))
}
samp_names<-names(prs[[1]])
dt<-data.table(SUBJID=samp_names, PRS_0=NA)
...
saveRDS(final2, paste0('~/height_prediction/loc_anc_analysis/output/all_PRS_UKB_afr_Eur_', args[1], '.Rds'))
...
prs<-vector('list', 22)
for(chr in 1:22){
        prs[[chr]]<-readRDS(paste0('~/height_prediction/loc_anc_analysis/output/chr', chr,'_0_prs_HRS_afr_EuR_',args[1],'.Rds'))
}
samp_names<-names(prs[[1]])
dt<-data.table(SUBJID=samp_names, PRS_0=NA)
