...
###########################################
#args<-c('phys_100000_0.0005') # arguments should be: pruned set, chromosome, alpha #comment this line
chr<-args[2]
###read in PRS SNPs for HRS_afr
hei<-readRDS(paste0('~/height_prediction/gwas/HRS_afr/output/hei_', args[1], '_v2.Rds'))[[chr]]
...
what <- paste0("/project/mathilab/data/HRS/local_ancestry/RFMix/HRS_AFR_b37_strand_include_kgCY_chr", chr)
ind<-read.table(paste0('/project/mathilab/data/HRS/data/phased/hapi-ur/HRS_AFR_b37_strand_include_kgCY_chr', chr, ".phind"), as.is=TRUE)
geno <- read_fwf(paste0('/project/mathilab/data/HRS/data/phased/hapi-ur/HRS_AFR_b37_strand_include_kgCY_chr', chr, ".phgeno"), fwf_widths(rep(1,NROW(ind))))
gc()
ancestry <- read_fwf(paste0(what, "_rfmix_out.0.Viterbi.txt.gz"), fwf_empty(paste0(what, "_rfmix_out.0.Viterbi.txt.gz")))
gc()
snp<-fread(paste0('/project/mathilab/data/HRS/data/phased/hapi-ur/HRS_AFR_b37_strand_include_kgCY_chr', chr,  ".phsnp"))
...
merge(snp1, hei, by=c('CHR', 'POS'), sort=F)->hei1 #another test here: hei1[REF_snp==ALT_hei] all lines are true
gc()
...
saveRDS(RES2, file=paste0('~/height_prediction/loc_anc_analysis/output/chr', chr,'_prs_HRS_afr_EuR.Rds'))
# print elapsed time
