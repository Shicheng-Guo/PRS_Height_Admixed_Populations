...
eurA<-dataA[Anc==2] #EUR anc is 2. Select only those positions
res<-sum(eurA$PRS_part, na.rm=T)
return(res)
}

system.time(RES<-lapply(1:ncol(geno2), function(I) LA_PRS(X2=I))) ### 66 seconds for chr22
...
saveRDS(RES2, file=paste0('~/height_prediction/loc_anc_analysis/output/chr', chr, '_prs_WHI_EuR.Rds'))
# print elapsed time
