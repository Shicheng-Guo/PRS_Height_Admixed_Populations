...
#read in PGS scores
readRDS('~/height_prediction/random_effect_prs/output/PGS_ukb_eur.Rds')-> PGS_UKB_eur
...
saveRDS(A_table, file='~/height_prediction/random_effect_prs/output/Nr_SNPs_UKB_eur.Rds')
...
for(I in 1:length(PGS2_UKB_eur)){
        ...
        png(filename=paste0('~/height_prediction/random_effect_prs/figs/UKB_eur_ggpairs_', names(PGS2_UKB_eur)[I], '.png'))
        print(A)
...
#lapply(PGS2_UKB_eur, function(X) X[, Quantile:= cut(EUR_ANC,
#                                breaks=quantile(EUR_ANC, probs=seq(0,1, by=0.25), na.rm=TRUE),
#                                include.lowest=TRUE)])-> PGS3_UKB_eur

#names(PGS3_UKB_eur)<-names(PGS2_UKB_eur)

#lapply(1:length(PGS3_UKB_eur), function(X) PGS3_UKB_eur[[X]][,Med_Eur_Anc:=median(EUR_ANC),by=Quantile])

#lapply(1:length(PGS3_UKB_eur), function(X) as.character(unique((PGS3_UKB_eur[[X]]$Quantile))))-> a

#lapply(a, function(X) c(X[2],X[4], X[3], X[1]))-> a1 #check

#names(a1)<-names(PGS3_UKB_eur)

lapply(PGS2_UKB_eur, function(X) X[, Quantile:='total'][,Med_Eur_Anc:=1])-> PGS3_UKB_eur

#r2_UKB_eur<-vector('list', length(PGS3_UKB_eur))
#names(r2_UKB_eur)<-names(PGS3_UKB_eur)

#for(I in names(r2_UKB_eur)){
#        r2_UKB_eur[[I]]<-vector('list', length(a1[[I]]))
#        names(r2_UKB_eur[[I]])<-a1[[I]]
#        for(i in a1[[I]]){
#        r2_UKB_eur[[I]][[i]]<-partial.R2(lm(Height~Sex+Age+age2+EUR_ANC, PGS3_UKB_eur[[I]][Quantile==i]),lm(Height~Sex+Age+age2+EUR_ANC+PGS, PGS3_UKB_eur[[I]][Quantile==i]))
#        }
#}
...
#for (I in names(PGS3_UKB_eur)){
#results.UKB_eur[[I]]<-vector('list', length(a1[[I]])+1)
#        lapply(a1[[I]], function(i) boot(data=PGS3_UKB_eur[[I]][Quantile==i], statistic=rsq.R2,
#                R=999, formula1=Height~Sex+Age+age2+EUR_ANC, formula2=Height~Sex+Age+age2+EUR_ANC+PGS))-> results.UKB_eur[[I]]
#        cat(I)
#        cat(' done\n')
#}
...
saveRDS(PGS3_UKB_eur, file='~/height_prediction/random_effect_prs/output/PGS3_UKB_eur.Rds')
saveRDS(results.UKB_eur, file='~/height_prediction/random_effect_prs/output/results.UKB_eur.Rds')
...
saveRDS(B_UKB_eur, file="~/height_prediction/random_effect_prs/output/B_UKB_eur.Rds")
