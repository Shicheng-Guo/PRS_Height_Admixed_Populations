...
snps <- read.table(paste0("UKB_kgCY_chr", chr, ".phsnp"), as.is=TRUE)
...
#betas$y2<- betas$POP1-betas$POP2
#At this point you will restrict the betas to the SNPS that you are using in the PRS. Just for example, I'm restricting to the top 100 ALL betas instead.
#betas <- betas[order(-abs(betas$ALL)),][1:100,]
betas<-betas[which(betas$POS %in% ID_posgrch37$POS),] #this is also not ideal. 
setkey(betas, CHR, POS)
setkey(snp_list, CHR, POS)
betas[snp_list[,.(CHR, POS, b)]]-> betas
...
#bmodel <- sapply(unique(beta$CHR), function(X) lm(beta[CHR==X]$y~beta[CHR==X]$AA.rate))
#ggplot(beta, aes(x=AA.rate, y=y, group=CHR, colour=CHR)) + 
#geom_point() + 
#geom_line(bmodel)
...
ggplot(beta, aes(x=CEU_YRI_diff.rate,y=y2)) + geom_point() + geom_smooth(method='lm') + ggtitle(paste0(args[2], " distance"))
dev.off()

pdf(paste0("~/height_prediction/gwas/ukb_afr/figs/all_chr_rate_against_AA_map_combined", args[2], ".pdf"))
ggplot(beta[AA.rate<=0.05], aes(x=AA.rate, y=y2)) + geom_point() + geom_smooth(method='lm') + ggtitle(paste0(args[2], " distance"))
dev.off()

pdf(paste0("~/height_prediction/gwas/ukb_afr/figs/all_chr_rate_against_AA_map_combined", args[2], ".pdf"))
ggplot(beta[CEU.rate<=0.05], aes(x=AA.rate, y=y2)) + geom_point() + geom_smooth(method='lm') + ggtitle(paste0(args[2], " distance"))
dev.off()
