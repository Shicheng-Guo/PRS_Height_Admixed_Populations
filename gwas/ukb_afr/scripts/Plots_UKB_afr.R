...
txtStart(paste0("~/height_prediction/gwas/ukb_afr/plots_out.txt"))
#read in PGS scores
readRDS('~/height_prediction/gwas/ukb_afr/output/PGS_ukb_afr.Rds')-> PGS_UKB_afr
...
saveRDS(A_table, file='~/height_prediction/gwas/ukb_afr/output/Nr_SNPs_UKB_afr.Rds')

#
cor.test(unlist(Nr_SNPs), unlist(partial_r2_UKB_afr))# 0.64
summary(lm(Part_R2~Nr,data=A_table))$r.squared #0.41

for(I in 1:length(PGS2_UKB_afr)){
        A<-ggpairs(PGS2_UKB_afr[[I]][,.(Height, Sex, PGS, Age, age2,EUR_ANC)])
        png(filename=paste0('~/height_prediction/gwas/ukb_afr/figs/UKB_afr_ggpairs_', names(PGS2_UKB_afr)[I], '.png'))
        print(A)
...
saveRDS(PGS3_UKB_afr, file='~/height_prediction/gwas/ukb_afr/output/PGS3_UKB_afr.Rds')
saveRDS(results.UKB_afr, file='~/height_prediction/gwas/ukb_afr/output/results.UKB_afr.Rds')
...
for (I in names(PGS3_UKB_afr)){
       myp<-ggplot(B_UKB_afr[[I]][1:4,], aes(x=Med_Eur_Anc, y=R_sq)) +
        ...
        print(myp)
        ggsave(paste0('~/height_prediction/gwas/ukb_afr/figs/UKB_afr_error_bars_', I, '.png'))
}

saveRDS(B_UKB_afr, file="~/height_prediction/gwas/ukb_afr/output/B_UKB_afr.Rds")
txtStop()
