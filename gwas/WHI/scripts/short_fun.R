...
short_fun<-function(args=22, data=prun2){
        what <- paste0("/project/mathilab/data/UKB/phased/hapi-ur/UKB_kgCY_chr", args)
...
short_fun_v2<-function(args=22, data=prun2){
	what<-paste0("/project/mathilab/data/UKB/phased/hapi-ur/UKB_kgCY_chr", args)
        betas<-fread(paste0('~/height_prediction/gwas/ukb_afr/output/AS_Beta_chr', args,'example.txt'))
        prun<-setDT(readRDS('~/height_prediction/gwas/HRS_afr/output/hei_phys_100000_0.0005_v2.Rds')[[args]])
        fread(paste0(what, '.phsnp'))-> snps
        colnames(snps)<-c("ID", "CHR", "V3", "POS","REF","ALT")
        tag='phys_100000_0.0005'
        cbind(betas, snps)-> betas2
...
short_fun_v3<-function(args=22, data=prun2){
	what<-paste0("/project/mathilab/data/UKB/phased/hapi-ur/UKB_kgCY_chr", args)
        betas<-fread(paste0('~/height_prediction/gwas/ukb_afr/output/AS_Beta_chr', args,'example.txt'))
        prun<-setDT(readRDS('~/height_prediction/gwas/JHS/output/hei_phys_100000_0.0005_v2.Rds')[[args]])
        fread(paste0(what, '.phsnp'))-> snps
        colnames(snps)<-c("ID", "CHR", "V3", "POS","REF","ALT")
        tag='phys_100000_0.0005'
        cbind(betas, snps)-> betas2
