PolScore3<- function(beta='b', data=prun2){
        samps<-colnames(data)[9:(ncol(data)-8)]
        setDT(data)
        #hei2<-hei[,c(1:5,10:7301)]
        cat('checkpoint \n')
	data[ALT==Allele1]-> temp1
        data[REF==Allele1]-> temp2 #im ignoring the other two rows for now
...
PolScore3_imputed<- function(beta='PLINK', data=prun2){
        samps<-colnames(data)[9:(ncol(data)-10)]
        setDT(data)
        #hei2<-hei[,c(1:5,10:7301)]
        cat('checkpoint \n')
        data[ALT==Effect_Allele_plink]-> temp1
        data[REF==Effect_Allele_plink]-> temp2 #im ignoring the other two rows for now
        if(nrow(temp1)>0 & nrow(temp2)>0){
                matrix(nrow=nrow(temp1)+nrow(temp2), ncol=length(samps))-> my_matrix
                colnames(my_matrix)<-samps
                rownames(my_matrix)<-c(temp1[,MarkerName],temp2[,MarkerName])
