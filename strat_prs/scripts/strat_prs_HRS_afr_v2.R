...
afr<-do.call(rbind, readRDS(paste0('~/height_prediction/', args[3],'/HRS_afr/output/hei_phys_100000_0.0005_v2.Rds')))[,.(CHR,POS,MarkerName,MarkerName,REF,ALT,Allele1,Allele2,b,p)]
...
saveRDS(afr, file=paste0("~/height_prediction/strat_prs/output/rec_quant_HRS_afr_",args[3], '_',rate.dist, "_", w_map, "_v2.Rds")) #store results
#Make a list
...
lapply(c("q1","q2","q3","q4"), function(I) boot(data=PGS2_HRS_afr[[I]], statistic=rsq.R2,R=3000, formula1=HEIGHT~SEX+AGE+AGE2+EUR_ANC, formula2=HEIGHT~SEX+AGE+AGE2+EUR_ANC+PGS))-> results.HRS_afr
...
saveRDS(boots.ci.HRS_afr, file=paste0('~/height_prediction/strat_prs/output/results_HRS_afr_', args[3], '_', rate.dist,"_", w_map, '_v2.Rds'))
