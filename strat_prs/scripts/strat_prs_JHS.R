...
w_map<-args[2]
PRS<-vector('list', 22)
jhs<-fread('/project/mathilab/bbita/gwas_admix/new_height/jhs_betas_100000_0.0005.txt')  #need to fix this path
...
hei<-lapply(1:22, function(chr) readRDS('/project/mathilab/bbita/gwas_admix/new_height/JHS/hei_phys_100000_0.0005_v2.Rds')[[chr]]) #need to fix this path
...
PRS<-prs
remove(prs)
saveRDS(PRS,file=paste0("../output/prs_JHS_", rate.dist, "_", w_map, ".Rds")) #store results.
obj<-c(nrow(hei_q1), nrow(hei_q2), nrow(hei_q3), nrow(hei_q4))
saveRDS(obj, file=paste0("..output/Nr_SNPs_JHS_", rate.dist, "_", w_map, ".Rds")) #store results.
...
saveRDS(PRS2, file=paste0('../output/PRS2_JHS_',rate.dist,"_", w_map, '.Rds')) #store results.
...
fread('/project/mathilab/bbita/gwas_admix/new_height/JHS/JHS_phenotypes.txt')-> Pheno_JHS #need to fix this path
...
anc_JHS<-cbind(fread('/project/mathilab/bbita/gwas_admix/new_height/JHS/JHS_b37_strand_prune.2.Q'), fread('/project/mathilab/bbita/gwas_admix/new_height/JHS/JHS_b37_strand_prune.fam')[,V1]) #need to fix this path
...
saveRDS(partial_r2_JHS,file=paste0('../output/part_R2_JHS_', rate.dist, "_", w_map, '.Rds')) #store results
