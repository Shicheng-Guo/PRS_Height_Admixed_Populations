...
chr<-args[2]
###read in PRS SNPs for UKB_afr
hei<-readRDS(paste0('~/height_prediction/gwas/ukb_afr/output/hei_', args[1], '_v2.Rds'))[[chr]]
...
what <- paste0("/project/mathilab/data/UKB/local_ancestry/RFMix/UKB_kgCY_chr", chr)
ind<-read.table(paste0('/project/mathilab/data/UKB/phased/hapi-ur/UKB_kgCY_chr', chr, ".phind"), as.is=TRUE)
geno <- read_fwf(paste0('/project/mathilab/data/UKB/phased/hapi-ur/UKB_kgCY_chr', chr, ".phgeno"), fwf_widths(rep(1,NROW(ind))))
gc()
ancestry <- read_fwf(paste0(what, "_rfmix_out.0.Viterbi.txt.gz"), fwf_empty(paste0(what, "_rfmix_out.0.Viterbi.txt.gz")))
gc()
snp<-fread(paste0('/project/mathilab/data/UKB/phased/hapi-ur/UKB_kgCY_chr', chr,  ".phsnp"))
...
saveRDS(RES2, file=paste0('~/height_prediction/loc_anc_analysis/output/chr', chr,'_prs_UKB_afr_EuR.Rds'))
# print elapsed time
