...
dplyr::select(hei, -c(QUAL, FILTER,INFO, FORMAT, N, SE))-> hei #remove some useless columns
gc()
...
hei %>% dplyr::select(-contains("0_")) %>% as.data.table-> hei #Allele1 is always the ALT allele. If we check hei[ALT_hei==Allele1] we will see that that's not the case for all positions, showing that ref_hei/alt_hei are not reliable for ref/alt info.
...
saveRDS(RES2, file=paste0('~/height_prediction/loc_anc_analysis/output/chr', chr,'_', args[3], '_prs_HRS_afr_', args[4], '.Rds'))
# print elapsed time
