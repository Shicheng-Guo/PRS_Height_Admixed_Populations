...
PRS<-prs
remove(prs)
saveRDS(PRS,file=paste0("../output/prs_pennBB_eur_", rate.dist, "_", w_map, ".Rds")) #store results
obj<-c(nrow(hei_q1), nrow(hei_q2), nrow(hei_q3), nrow(hei_q4))
saveRDS(obj, file=paste0("../output/Nr_SNPs_pennBB_eur_", rate.dist, "_", w_map, ".Rds")) #store results
...
saveRDS(PRS2, file=paste0('../output/PRS2_pennBB_eur_',rate.dist,"_", w_map, '.Rds'))  #store results
...
Pheno2_pennBB_eur[,GENO_ID:=paste0(GENO_ID, "_", GENO_ID)]
setkey(Pheno_pennBB_eur, GENO_ID)
setkey(Pheno2_pennBB_eur, GENO_ID)
...
PGS2_pennBB_eur<-vector('list', 4)
names(PGS2_pennBB_eur)<-c("q1","q2","q3","q4")
for (I in names(PGS2_pennBB_eur)){
        data.table(GENO_ID=rownames(PRS2), PGS=PRS2[,get(I)])-> PGS2_pennBB_eur[[I]]
        setkey(PGS2_pennBB_eur[[I]], GENO_ID)
        PGS2_pennBB_eur[[I]][Pheno3_pennBB_eur, nomatch=0]-> PGS2_pennBB_eur[[I]]
        PGS2_pennBB_eur[[I]][anc_pennBB_eur, nomatch=0]-> PGS2_pennBB_eur[[I]]
        PGS2_pennBB_eur[[I]][,HEIGHTX:=Height*2.54] #convert to cm
        PGS2_pennBB_eur[[I]][,WEIGHTX:=Weight*0.45359237] #convert to kg
        PGS2_pennBB_eur[[I]][-which(is.na(PGS2_pennBB_eur[[I]][,HEIGHTX])),]-> PGS2_pennBB_eur[[I]]
}

#run linear models
lapply(PGS2_pennBB_eur, function(X) lm(HEIGHTX~sex,X))-> lm0_pennBB_eur
lapply(PGS2_pennBB_eur, function(X) lm(HEIGHTX~PGS, X))-> lm1_pennBB_eur
lapply(PGS2_pennBB_eur, function(X) lm(HEIGHTX~age, X))-> lm2_pennBB_eur
lapply(PGS2_pennBB_eur, function(X) lm(HEIGHTX~age2, X))-> lm3_pennBB_eur
lapply(PGS2_pennBB_eur, function(X) lm(HEIGHTX~EUR_ANC, X))-> lm4_pennBB_eur
lapply(PGS2_pennBB_eur, function(X) lm(HEIGHTX~sex+age, X))-> lm5_pennBB_eur
lapply(PGS2_pennBB_eur, function(X) lm(HEIGHTX~sex+age+age2, X))-> lm6_pennBB_eur
lapply(PGS2_pennBB_eur, function(X) lm(HEIGHTX~sex+age+age2+EUR_ANC, X))-> lm7_pennBB_eur
lapply(PGS2_pennBB_eur, function(X) lm(HEIGHTX~sex+age+age2+EUR_ANC+PGS,X))-> lm8_pennBB_eur

#Get partial R2, i.e, the proportion of variation in height explained by the PRS
partial_r2_pennBB_eur<-lapply(1:length(PGS2_pennBB_eur), function(X) partial.R2(lm7_pennBB_eur[[X]], lm8_pennBB_eur[[X]])) 
names(partial_r2_pennBB_eur)<- names(PGS2_pennBB_eur)

saveRDS(partial_r2_pennBB_eur,file=paste0('../output/part_R2_pennBB_eur_', rate.dist, "_", w_map, '.Rds')) #store results.
