...
b1<-paste0('~/height_prediction/ldpred/output/UKB_EUR.score_LDpred', b, '.txt')

d<-c(unlist(a1), unlist(b1))
lapply(d , function(X) fread(X))-> PGS_UKB_eur
...
my_names<-c(paste0('pt_', a), paste0('gibbs_', b))
names(PGS_UKB_eur)<-my_names
...
saveRDS(B_UKB_eur, file="~/height_prediction/ldpred/output/B_UKB_eur_all.Rds")
