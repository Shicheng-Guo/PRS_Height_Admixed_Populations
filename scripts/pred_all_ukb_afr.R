...
a1<-paste0('~/height_prediction/ldpred/output/UKB_AFR.score_P+T_r0.20',a, '.txt')

b<-c('_p1.0000e-03', '_p3.0000e-03','_p1.0000e-02', '_p3.0000e-02', '_p1.0000e-01', '_p3.0000e-01','_p1.0000e+00', '-inf')

b1<-paste0('~/height_prediction/ldpred/output/UKB_AFR.score_LDpred', b, '.txt')
...
for (I in names(PGS2_UKB_afr)){
	results.UKB_afr[[I]]<-vector('list', length(a1[[I]])+1)
        lapply(a1[[I]], function(i) boot(data=PGS2_UKB_afr[[I]][Quantile==i], statistic=rsq.R2,
                R=1000, formula1=Height~Sex+Age+age2+EUR_ANC, formula2=Height~Sex+Age+age2+EUR_ANC+PRS))-> results.UKB_afr[[I]]
        cat(I)
        cat(' done\n')
}

for (I in names(PGS2_UKB_afr)){
        tp <- boot(data=PGS2_UKB_afr[[I]], statistic=rsq.R2, R=1000, formula1=Height~Sex+Age+age2+EUR_ANC, formula2=Height~Sex+Age+age2+EUR_ANC+PRS)
        list.append(results.UKB_afr[[I]], tp)-> results.UKB_afr[[I]]
        names(results.UKB_afr[[I]])<-c(a1[[I]], "total")
        cat(' done\n')
}
saveRDS(PGS2_UKB_afr, file='~/height_prediction/ldpred/output/PGS3_UKB_afr.Rds')
saveRDS(results.UKB_afr, file='~/height_prediction/ldpred/output/results.UKB_afr.Rds')
...
saveRDS(B_UKB_afr, file="~/height_prediction/ldpred/output/B_UKB_afr.Rds")
