...
args<-c('phys_100000_0.0005', 20, 0) # arguments should be: pruned set, chromosome, alpha #comment this line
chr<-args[2]
final_plink<-as.data.table(readRDS('~/height_prediction/loc_anc_analysis/output/final_plink.Rds'))[CHR==chr] #plink output for UKB_AFR gwas. Generated by combine_plink_files.R
...
ancestry1<-ancestry[which(snp$POS %in% hei$POS),]
#
...
#test: for indiv 0_700001, remove these positions (missing genotype)
source('~/height_prediction/gwas/WHI/scripts/PolygenicScore_v3.R')
c('21336825', '22065466', '32714728', '50266043', '51175483', '51175483')
snp3<-snp2[-which(snp2$POS %in%  c(21336825, 22065466, 32714728, 50266043, 51175483, 51175483)),]
geno3<-geno2[-which(snp2$POS %in%  c(21336825, 22065466, 32714728, 50266043, 51175483, 51175483)),]
anc3<-anc2[-which(snp2$POS %in%  c(21336825, 22065466, 32714728, 50266043, 51175483, 51175483)),]
plink_prun2<-plink_prun[-which(snp2$POS %in%  c(21336825, 22065466, 32714728, 50266043, 51175483, 51175483)),]

LA_PRS<- function(X=geno3[1:126,], X2=1, Y=anc3[1:126,], alpha=as.numeric(args[3])){
dataA<-cbind(data.table(Anc=unlist(Y[,X2]), plink_prun2[1:126,]))[, Geno:=ifelse(AlM=='YES', (1-unlist(X[,X2])), unlist(X[,X2]))] ##if YES, 0=REF and 1=ALT. If NO, 1=REF, 0=ALT, thus 1-Geno
afrA<-dataA[Anc==1]
eurA<-dataA[Anc==2]
if(nrow(afrA)>=1){
...
LA_PRS(X2=1)+LA_PRS(X2=2)

saveRDS(plink_prun2[1:126,MarkerName], file="test.Rds")
#readRDS('~/height_prediction/gwas/WHI/output/PGS_chrs_phys_100000_0.0005.Rds')[[20]]['0_700001']


> prun2<-setDT(readRDS('~/height_prediction/gwas/WHI/output/hei_phys_100000_0.0005_v2.Rds')[[args]])[i.MarkerName %in% readRDS('~/height_prediction/loc_anc_analysis/test.Rds')]
> PolScore3(beta='b', data=prun2)['0_700003']
