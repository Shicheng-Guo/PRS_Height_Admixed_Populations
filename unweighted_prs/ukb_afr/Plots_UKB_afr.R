...
txtStart(paste0("~/height_prediction/unweighted_prs/ukb_afr/plots_out.txt"))

#read in PGS scores
readRDS('~/height_prediction/unweighted_prs/output/PGS_ukb_afr.Rds')-> PGS_UKB_afr
...
partial.R2(lm7_UKB_afr[[35]],lm8_UKB_afr[[35]]) 
...
for(I in names(Nr_SNPs)){
        readRDS(paste0('~/height_prediction/gwas/ukb_afr/output/hei_', I, '.Rds'))-> smtg
...
saveRDS(A_table, file='~/height_prediction/unweighted_prs/output/Nr_SNPs_UKB_afr.Rds')

#
cor.test(unlist(Nr_SNPs), unlist(partial_r2_UKB_afr))# 
...
for(I in 1:length(PGS2_UKB_afr)){
        ...
        png(filename=paste0('~/height_prediction/unweighted_prs/figs/UKB_afr_ggpairs_', names(PGS2_UKB_afr)[I], '.png'))
        print(A)
...
saveRDS(PGS3_UKB_afr, file='~/height_prediction/unweighted_prs/output/PGS3_UKB_afr.Rds')
saveRDS(results.UKB_afr, file='~/height_prediction/unweighted_prs/output/results.UKB_afr.Rds')
...
for (I in names(PGS3_UKB_afr)){
       myp<-ggplot(B_UKB_afr[[I]][1:4,], aes(x=Med_Eur_Anc, y=R_sq)) +
        ...
        print(myp)
        ggsave(paste0('~/height_prediction/unweighted_prs/figs/UKB_afr_error_bars_', I, '.png'))
}

saveRDS(B_UKB_afr, file="~/height_prediction/unweighted_prs/output/B_UKB_afr.Rds")
txtStop()
